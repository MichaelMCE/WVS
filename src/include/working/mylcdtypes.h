...
typedef struct{
	...
	unsigned int devcount;	// total activated displays
	int flags[CAPS_TOTAL];	// various flags
	TTRENDER *render;
}THWD;
...
typedef struct {
	...
	int (*set)		(const TFRAME *frm, const int x, const int y, const int colour);
	...
	int (*set_NB)	(const TFRAME *frm, const int x, const int y, const int colour);
	...
	int (*setf)		(const TFRAME *frm, const int x, const int y, float  r, float  g, float  b);
	...
	//row = y * pitch
	int (*set_NBr)	(const TFRAME *frm, const int x, const int row, const int colour);

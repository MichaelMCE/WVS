...
TASCIILINE *readFileA (const char *filename)
{
	...
	al->tlines = 256;
	al->lines = (ubyte **)calloc(1+al->tlines, sizeof(ubyte *));
	if (!al->lines){
		free(al->data);
		free(al);
		return NULL;
	...
	do{
		...
		if (i >= al->tlines){
			al->tlines <<= 1;
			al->lines = (ubyte **)realloc(al->lines, (1+al->tlines)* sizeof(ubyte *));
			if (al->lines == NULL){
	...
	if (i < al->tlines){
		al->lines = (ubyte **)realloc(al->lines, (1+i) * sizeof(ubyte *));
		if (al->lines == NULL){
			printf(": readFileA: realloc returned null\n");
			return NULL;
		}

...
!define PRODUCT_VERSION "0.30.14 for MediaMonkey"
...
; Finish page
!define MUI_FINISHPAGE_RUN "$INSTDIR\..\MediaMonkey.exe"
...
OutFile "wvs_03014-mediamonkey-setup.exe"
InstallDir "$PROGRAMFILES\MediaMonkey\Plugins"
...
loop:
  push "MediaMonkey.exe"
  processwork::existsprocess
  pop $5
  IntCmp $5 0 done

  MessageBox MB_RETRYCANCEL|MB_ICONSTOP 'MediaMonkey must be closed during this installation.$\r$\n Close MediaMonkey now, or press "Retry" to automatically close MediaMonkey and continue or press "Cancel" to cancel the installation entirely.' IDCANCEL BailOut
  push "MediaMonkey.exe"
...
loop:
  push "MediaMonkey (non-skinned).exe"
  processwork::existsprocess
  pop $5
  IntCmp $5 0 done
...
; The stuff to install
  ...
  Call ClosePlayer1
  Call ClosePlayer2
  ; add a small delay to allow any file operations to happen once Player
  ; is closed
...
;  SetOutPath "$INSTDIR\wvs\CopyToMediaMonkeyPlugins"
  ...
  File "t:\wvssetup\wvs\CopyToMediaMonkeyPlugins\gen_wvs.dll"
...
;  Delete "$INSTDIR\wvs\CopyToXMPlay\xmp-wvs.dll"
  ...
  Delete "$INSTDIR\vis_wamp.dll"
  Delete "$INSTDIR\gen_wamp.dll"
  Delete "$INSTDIR\vis_wvs.svp"
  Delete "$INSTDIR\xmp-wvs.dll"
  Delete "$INSTDIR\gen_wvs.dll"
